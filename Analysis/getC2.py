...
base_names = ['DSI2', 'MAN2']#, 'LIB1', 'LIB2', 'MAN2']
base_dir = "/mnt/c/Users/hamaa/OneDrive - Università degli Studi di Catania/PHD code/transformer_practice/University_valencia_IP/Lets_talk_about_svm/Results_simulations/plainSVRm_2024"
results_dir = "/mnt/c/Users/hamaa/OneDrive - Università degli Studi di Catania/PHD code/transformer_practice/University_valencia_IP/Lets_talk_about_svm/"
train_test_splits = [0.1, 0.2]
data_type = "scaled"
repetitions = 2
max_iter = -1
config_name = "C2"
...
# # Function to save the error and predictions CSV files
# def save_repetition_files(config_name, dataset, config_folder, repetitions):
#     # Create directory to store CSV files for this configuration and dataset
#     save_folder_path = os.path.join(results_dir, "Results and Analysis", "plainSVRm_2024", config_name, dataset, config_folder)
#     os.makedirs(save_folder_path, exist_ok=True)

#     # Loop through repetitions to save files
#     for i in range(1, repetitions + 1):
#         # Define file paths
#         error_file_path = os.path.join(base_dir, dataset, config_folder, f"error_rep{i}.csv")
#         prediction_file_path = os.path.join(base_dir, dataset, config_folder, f"predictions_rep{i}.csv")
        
#         # Check if error file exists and save it
#         if os.path.exists(error_file_path):
#             error_df = pd.read_csv(error_file_path)
#             error_df.to_csv(os.path.join(save_folder_path, f"error_rep{i}.csv"), index=False)
        
#         # Check if prediction file exists and save it
#         if os.path.exists(prediction_file_path):
#             prediction_df = pd.read_csv(prediction_file_path)
#             prediction_df.to_csv(os.path.join(save_folder_path, f"predictions_rep{i}.csv"), index=False)
