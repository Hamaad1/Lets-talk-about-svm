...
Output Directory: ../Lets_talk_about_svm/Results and Analysis
    ...
    ###################################################################################################################################################
    B.  Lets Talk about the Functions used in getCi_cmd.py: A command-based analysis in the analysis folder
    ###################################################################################################################################################
    ...
    1. copy_config_files()
        ...
        It will generate a folder structure like: output directory/plainSVRs_2024/config_name/dataset_name/configuration_folder/error_rep*.csv and prediction_rep*.csv files.
        This function is commented out in save_summary_files(), as it takes a little time to copy for big configuration, uncomment it the first time to get the files, and then comment it again to save time.
    ...
    Then comment on it to save time.
    ...
    In this file, I am analyzing the individually given combination so that I can get the output analysis for the given configuration only
    ...
    ###################################################################################################################################################
    progress_file.json
        It is the JSON file that contains the progress of the models. It records each processed combination during the exhaustive search. It will help us in tracking the  combinations
...
To use this file first change the directory to cd "right-click the folder that has this file to copy the path"
Press enter.
...
It will take the initial default values by it self like, processed dataset, redult_directory, data_directory, and log_file path.
